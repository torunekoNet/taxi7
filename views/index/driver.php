<?php
/**
 * File: index.php
 * User: daijianhao(toruneko@outlook.com)
 * Date: 2017/12/9 13:41
 * Description:
 */
?>

<!DOCTYPE html>
<html>
<head>
    <title>Taxi7 管理系统 - 驾驶员</title>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link rel="stylesheet" href="//assets.toruneko.net/taxi7/icon/iconfont.css?v=<?php echo $jsVersion ?>">
    <link rel="stylesheet" href="//assets.toruneko.net/taxi7/driver/driver.css?v=<?php echo $jsVersion ?>">
    <script src="//assets.toruneko.net/taxi7/dll/vendor1.js?v=<?php echo $jsVersion ?>"></script>
    <script src="//assets.toruneko.net/taxi7/dll/vendor2.js?v=<?php echo $jsVersion ?>"></script>
</head>
<body>
<div id="app">
    <div class="td-layout">
        <div class="td-layout td-layout-has-sider">
            <div id="refrain-content" class="td-layout-content"></div>
        </div>
    </div>
</div>
<script src="//assets.toruneko.net/taxi7/driver/driver.js?v=<?php echo $jsVersion ?>"></script>
</body>
</html>
